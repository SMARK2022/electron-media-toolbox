...
// 获取所有详细照片记录，包括新增列
export function getPhotosExtend() {
    const sql = `
        SELECT *
        FROM present
    `;
...
// 获取所有启用的照片记录
export function getEnabledPhotosExtend() {
    const sql = `
        SELECT *
        FROM present
        WHERE isEnabled = 1
...
// 获取单个照片的详细记录
export function getPhotoExtendByPhoto(photo: Photo) {
    const sql = `
        SELECT *
        FROM present
        WHERE fileName = @fileName AND  filePath = @filePath 
    `;
    return window.ElectronDB.get(sql, photo); // get 方法期望传递 SQL 和参数，返回单条记录
}
...
// 获取多个照片的详细记录
export function getPhotosExtendByPhotos(photos: Photo[]) {
    const results = [];
    for (const photo of photos) {
        const result = getPhotoExtendByPhoto( photo); // get 方法期望传递 SQL 和参数，返回单条记录
        if (result) {
            results.push(result);
        }
    }
