...
export function PhotoGridEnhance({
...
}: PhotoGridProps & { onPhotoClick?: (photos: Photo[], event: string) => void; highlightPhotos?: Photo[] }) {
    // 当前焦点索引
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        ...
        switch (e.key) {
            ...
            case "ArrowDown":
                // 下移
                if (focusedIndex !== null && focusedIndex + 3 < photos.length && onPhotoClick) {
                    newFocusedIndex = focusedIndex + 3;
                    setFocusedIndex(newFocusedIndex); // 更新焦点
                    const selectedPhoto = photos[newFocusedIndex];
                    onPhotoClick([selectedPhoto], "Select"); // 调用选中回调
            ...
            case "ArrowRight":
                // 右移
                if (focusedIndex !== null && focusedIndex + 1 < photos.length && onPhotoClick) {
                    newFocusedIndex = focusedIndex + 1;
                    setFocusedIndex(newFocusedIndex); // 更新焦点
                    const selectedPhoto = photos[newFocusedIndex];
                    setHighlightPhotos([selectedPhoto]); // 更新 highlightPhotos
...
function LazyImageContainerExThumbs({ photo }: LazyImageContainerProps) {
    ...
    useEffect(() => {
        if (isVisible && photo.filePath) {
            const worker = new Worker(new URL("@/lib/thumbnail-worker.js", import.meta.url));
            ...
            worker.postMessage({ path: photo.fileUrl });
        }
    ...
    return (
        <Card>
            ...
            <div className="ml-1 space-y-1 text-sm">
                ...
                <p className="text-xs text-muted-foreground">{photo.info}</p>
            </div>
        </Card>
    );
}
