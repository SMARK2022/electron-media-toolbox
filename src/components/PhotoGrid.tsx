...
interface ContextMenuProps {
  ...
  groups: {
    id: string;
    label: string;
    items: { id: string; label: string; i18nKey?: string; icon?: string }[];
  }[];
}
 ...
 */
function isPhotoEqual(
  photoA: Photo | undefined,
  photoB: Photo | undefined,
): boolean {
 ...
 * 进行深度比较，检查数组长度和每个元素
 */
function arePhotoArraysEqual(
  arrA: Photo[] | undefined,
  arrB: Photo[] | undefined,
): boolean {
 ...
 */
function areGroupedPhotosEqual(
  groupsA: Photo[][] | undefined,
  groupsB: Photo[][] | undefined,
): boolean {
  if (groupsA === groupsB) return true;
  if (!groupsA || !groupsB) return groupsA === groupsB;
  if (groupsA.length !== groupsB.length) return false;
  return groupsA.every((groupA, idx) =>
    arePhotoArraysEqual(groupA, groupsB[idx]),
  );
}
...
const EyeStateBadge: React.FC<EyeStateBadgeProps> = React.memo(
  ({ eyeStats }) => {
    if (
      !eyeStats ||
      ...
      return null;
    const hasIssues =
      eyeStats.closedEyesCount > 0 || eyeStats.suspiciousCount > 0;
    return (
      <div className="absolute top-2 right-2 z-10 flex items-center gap-0 overflow-hidden rounded-md border border-white/10 bg-zinc-950/85 shadow-sm shadow-black/20 backdrop-blur-md select-none">
        {eyeStats.closedEyesCount > 0 && (
        ...
        {eyeStats.openEyesCount > 0 && (
          <div
            className={cn(
              "flex items-center gap-1 px-1 py-1 text-emerald-400 last:border-0",
            )}
          >
...
const PhotoCard: React.FC<PhotoCardProps> = React.memo(
  ({
    ...
    const { colorStyle, formattedInfo, showInfo } = useMemo(() => {
      ...
      return {
        colorStyle,
        formattedInfo: !Number.isNaN(numericInfo)
          ? numericInfo.toFixed(6)
          : infoStr,
        showInfo: !Number.isNaN(numericInfo) || infoStr !== "",
      };
    }, [photo.info]);
    ...
    return (
      ...
      >
        ...
        <div className="flex w-full flex-col gap-1 border-t border-gray-100 px-2 py-1.5 text-xs dark:border-slate-700">
          <p
            className="flex-1 truncate font-medium text-gray-700 dark:text-slate-200"
            title={photo.fileName}
          >
            {displayName}
          </p>
          {showInfo && (
            <p className="font-mono text-[11px]" style={{ color: colorStyle }}>
              {formattedInfo}
            </p>
          )}
        </div>
...
export const PhotoGridEnhance = React.memo(
  function PhotoGridEnhance({
    photos = [],
    ...
    const storeHighlightPaths = usePhotoFilterStore(
      (s) => s.highlightedPhotoFilePaths,
    ); // 高亮路径集合（store）

    // 本地 focus 状态（用于 import/export 页面，filter 页面使用 store）
    const [localFocusedPath, setLocalFocusedPath] = useState<string | null>(
      null,
    );
    const [localHighlightPaths, setLocalHighlightPaths] = useState<Set<string>>(
      new Set(),
    );
    ...
    const highlightedPaths = isFilterPage
      ? storeHighlightPaths
      : localHighlightPaths; // 高亮路径（综合 store + local）
      ...
      if (isGroupMode && groupedPhotos.length > 0) {
        ...
        groupedPhotos.forEach((group, gi) => {
          ...
          rows.push({
            type: "header",
            groupIndex: gi,
            label: `${t("filterPage.groupLabel") || "Group"} ${gi + 1}`,
          }); // 分组标题行
          for (
            let i = 0, rowIdx = 0;
            i < group.length;
            ...
            rows.push({
              ...
              groupIndex: gi,
              rowInGroup: rowIdx,
              photos: group.slice(i, i + columns),
            }); // 照片行
          }
        });
      } else {
        ...
        for (let i = 0; i < photos.length; i += columns) {
          rows.push({
            type: "photos",
            groupIndex: 0,
            rowInGroup: Math.floor(i / columns),
            photos: photos.slice(i, i + columns),
          });
        }
      }
      return rows;
    ...
    const estimateSize = useCallback(
      (idx: number) =>
        virtualRows[idx]?.type === "header" ? GROUP_HEADER_HEIGHT : ITEM_HEIGHT,
      [virtualRows],
    );
    ...
    const selectByPath = useCallback(
      (filePath: string, event: "Select" | "Change") => {
        // 在分组或平铺模式中查找照片
        ...
        if (isFilterPage) {
          ...
          if (event === "Change" && onPhotoClick)
            setTimeout(() => void onPhotoClick([targetPhoto], event), 0);
        }
    ...
    const handleKeyDown = useCallback(
      (e: React.KeyboardEvent) => {
        ...
        const rowIdx = Math.floor(pIdx / columns),
          colIdx = pIdx % columns; // 当前行列位置
        ...
        switch (e.key) {
          case "ArrowLeft": // 左移：同组内前一个 → 上一组末尾
            if (pIdx > 0)
              nextPhoto = group[pIdx - 1]; // 组内前一个
            else if (isGroupMode && gIdx > 0)
              nextPhoto = groupedPhotos[gIdx - 1].at(-1)!; // 上一组最后一个
            break;

          case "ArrowRight": // 右移：同组内后一个 → 下一组首个
            if (pIdx < group.length - 1)
              nextPhoto = group[pIdx + 1]; // 组内后一个
            else if (isGroupMode && gIdx < groupedPhotos.length - 1)
              nextPhoto = groupedPhotos[gIdx + 1][0]; // 下一组第一个
            break;

          case "ArrowUp": // 上移：同列上一行 → 上一组同列（不足则取末尾）
            if (rowIdx > 0)
              nextPhoto = group[(rowIdx - 1) * columns + colIdx]; // 同组上一行
            else if (isGroupMode && gIdx > 0) {
              // 跨组到上一组最后行同列
              const prev = groupedPhotos[gIdx - 1];
              const prevLastRowStart =
                Math.floor((prev.length - 1) / columns) * columns;
              nextPhoto =
                prev[Math.min(prevLastRowStart + colIdx, prev.length - 1)];
            }
            break;
      ...
      <>
          ...
          >
            ...
            {virtualRows.length > 0 ? (
                    ...
                    >
                      {row.photos?.map((photo) => (
                        <PhotoCard
                          ...
                          setRef={(el) => {
                            if (el) itemRefs.current.set(photo.filePath, el);
                            else itemRefs.current.delete(photo.filePath);
                          }}
                        />
                      ))}
                    </div>
              ...
              <div className="text-muted-foreground flex h-[calc(70vh-100px)] flex-col items-center justify-center text-center">
                <div className="mb-3 rounded-full bg-white p-4 shadow-sm dark:bg-slate-800">
          ...
          <ContextMenu
            ...
            onAction={(action) => {
              if (contextMenu.photo)
                fnHandleContextMenuAction(action, contextMenu.photo, page);
        ...
        <DeleteConfirmPortal
          ...
          onConfirm={async () => {
            ...
            if (objPendingDeletePhoto)
              await fnExecuteDeleteFile(objPendingDeletePhoto);
          }}
        />
  ...
  (prev, next) =>
    ...
    (prev.isGroupMode
      ? areGroupedPhotosEqual(prev.groupedPhotos, next.groupedPhotos)
      : arePhotoArraysEqual(prev.photos, next.photos)),
);
